...
Mesh* MeshBuilder::GenerateCylinder(const std::string& meshName, Color color, unsigned numSlice, float radius, float minHeight, float maxHeight)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (unsigned slice = 0; slice < numSlice; ++slice) {
		...
		float theta = slice * degreePerSlice;
		/*v0*/	v.pos.Set(radius * cos(Math::DegreeToRadian(theta)), minHeight, radius * sin(Math::DegreeToRadian(theta)));
		v.color = color;
		vertex_buffer_data.push_back(v);

		/*v1*/	v.pos.Set(radius * cos(Math::DegreeToRadian(theta)), maxHeight, radius * sin(Math::DegreeToRadian(theta)));
		v.color = color;
		vertex_buffer_data.push_back(v);

		float theta2 = (slice + 1) * degreePerSlice;
		/*v2*/	v.pos.Set(radius * cos(Math::DegreeToRadian(theta2)), minHeight, radius * sin(Math::DegreeToRadian(theta2)));
		v.color = color;
		vertex_buffer_data.push_back(v);

		/*v3*/	v.pos.Set(radius * cos(Math::DegreeToRadian(theta2)), maxHeight, radius * sin(Math::DegreeToRadian(theta2)));
		v.color = color;
		vertex_buffer_data.push_back(v);
	}
	for (unsigned slice = 0; slice < numSlice; ++slice) {
...
Mesh* MeshBuilder::GenerateCircle(const std::string& meshName, Color color, unsigned numSlice, float outerR)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateTerrain(const std::string& meshName,
	const std::string& file_path, std::vector<unsigned char>& heightMap)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (unsigned z = 0; z < terrainSize; ++z)
	{
		for (unsigned x = 0; x < terrainSize; ++x)
		{
			...
			v.texCoord.Set((float)x / terrainSize * 4, 1.f - (float)z / terrainSize * 4);
			v.normal.Set(0, 1, 0);
...
//Old code
//Mesh* MeshBuilder::GenerateSlicedQuad(const std::string& meshName, Color color, float x2, float z2)
//{
//	Vertex v;
//	std::vector<Vertex> vertex_buffer_data;
//	std::vector<GLuint> index_buffer_data;
...
//
//
//	unsigned terrainSize = 30;
//
//	for (unsigned z = 0; z < z2; ++z)
//	{
...
//			//float scaledHeight = (float)heightMap[z * terrainSize + x] / SCALE_FACTOR;
//
//			v.pos.Set(static_cast<float>(x) / x2 - 0.5f, 0, static_cast<float>(z) / z2 - 0.5f);
//			v.color = color;
//			v.texCoord.Set((float)x / x2 * 4, 1.f - (float)z / z2 * 4);
//			v.normal.Set(0, 1, 0);
//
//			vertex_buffer_data.push_back(v);
//		}
//	}
//
//	for (unsigned z = 0; z < z2 - 1; ++z)
//	{
//		for (unsigned x = 0; x < x2 - 1; ++x)
//		{
//			index_buffer_data.push_back(terrainSize * z + x + 0);            //Tri 1
//			index_buffer_data.push_back(terrainSize * (z + 1) + x + 0);
//			index_buffer_data.push_back(terrainSize * z + x + 1);
...
Mesh* MeshBuilder::GenerateSlicedQuad(const std::string& meshName, Color color, int numSlice, float h, float v, float scaleX, float scaleZ)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	float texDelta = 2.0f / (float)numSlice;

	for (float z = 0; z <= numSlice; ++z)
	{
		for (float x = 0; x <= numSlice; ++x)
		{

			Vertex VertexTemp; //Storing stuff to pushback
			VertexTemp.pos.x = (x / numSlice - 0.5f) * scaleX;
			VertexTemp.pos.y = 0;
			VertexTemp.pos.z = (z / numSlice - 0.5f) * scaleZ;

			// Calculate  texture coordinates
			VertexTemp.texCoord.u = h * ((float)x * texDelta * 0.5f);
			VertexTemp.texCoord.v = v * (1.0f - (float)z * texDelta * 0.5f);
			VertexTemp.color = color;
			vertex_buffer_data.push_back(VertexTemp);
		}
	}
	...
	for (int i = 0; i < numSlice; ++i)
	{
		...
		{
			int startvert = (i * (numSlice + 1) + j);
			index_buffer_data.push_back(startvert + numSlice + 1);
			index_buffer_data.push_back(startvert + numSlice + 2);
			index_buffer_data.push_back(startvert + 1); // tri 2
			index_buffer_data.push_back(startvert + numSlice + 1);
			index_buffer_data.push_back(startvert + 1);
			index_buffer_data.push_back(startvert); // tri 1
